...
@section('content')
        ...
        @foreach($categories as $category)
          ...
          <div class="row border-bottom bg-white">
                ...
                <div class="row mx-0 py-3">
                  ...
                  <div class="col-auto px-0 rounded-3">
                    <div class="p-0 me-1">
                      <div style="width: 6rem; height: 6rem;">
                        <div class="w-100 h-100 rounded-3 border border-body" style="background-image: url('/{{ $category->photo->path ?? 'img/default.png' }}'); background-size: cover; background-position: center center;"></div>
                      </div>
                    </div>
                  </div>
                  {{-- category name --}}
                  <div class="--sticky-table-item col d-flex flex-column justify-content-between">
                    <div class="row justify-content-between">
                      <div class="col-auto">
                        <p class="mb-0 fw-bolder text-break">{{ $category->nama }}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="col-3 py-3">
              <p class="--sticky-table-item mb-0 fw-bold" style="z-index: 1;">{{ $category->products()->count() }}</p>
            </div>
            ...
            <div class="col-2 py-3">
              <div class="--sticky-table-item" style="z-index: 1;">
                <div class="row">
                  <div class="col-auto">
                    <a href="#" data-bs-toggle="modal" data-bs-target="#modal-edit-category{{ $category->id }}"><i class="fas fa-edit"></i></a>
                  </div>
                  <div class="col-auto">
                    <a href="#" data-bs-toggle="modal" data-bs-target="#modal-delete-category{{ $category->id }}"><i class="fas fa-trash"></i></a>
                  </div>
                </div>
              </div>
            </div>
  ...
  @foreach($categories as $category)
    ...
    <div class="modal fade" id="modal-delete-category{{ $category->id }}" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <!-- modal: edit category -->
    <div class="modal fade" id="modal-edit-category{{ $category->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
          ...
          <form enctype="multipart/form-data" action="/manage-category/{{ $category->id }}" method="post">
            @method('put')
            @csrf
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col">
                  <div class="row mb-2">
                    <div class="col">
                      <p class="mb-0 fw-bold">Nama Kategori</p>
                      <input id="nama" name="nama" type="text" class="form-control" autocomplete="name" autofocus placeholder="Nama Kategori" value="{{ $category->nama }}" required>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col">
